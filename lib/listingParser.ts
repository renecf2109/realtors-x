...
export function parseListingDescription(text: string): ListingDraft {
  ...
  const priceMatch = lower.match(/(?:price|asking|rent|budget|for)\s*(?:is|of|:|-)?\s*(?:usd|\$)?\s*([\d,]+(?:\.\d+)?)/i)
    ?? lower.match(/(?:usd|\$)\s*([\d,]+(?:\.\d+)?)/i);
  const bedMatch = lower.match(/(\d+)\s*(?:bedrooms?|beds?|br)\b/i);
  const bathMatch = lower.match(/(\d+(?:\.\d+)?)\s*(?:bathrooms?|baths?|ba)\b/i);
  ...
  const size = sizeMatch ? cleanNumber(sizeMatch[1]) : undefined;
