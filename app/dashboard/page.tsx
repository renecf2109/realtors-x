...
export default async function Dashboard() {
  ...
  const [{ data: leads }, { count: listingCount }] = await Promise.all([
    ...
    <section>
      ...
      <div className="card mt-6 overflow-hidden">
        ...
        {typedLeads.length === 0 ? <p className="p-8 text-center text-ink/50">No leads yet. Share the public chat page to get started.</p> :
          <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="bg-cream text-xs uppercase text-ink/45"><tr><th className="p-4">Buyer</th><th className="p-4">Request</th><th className="p-4">Budget</th><th className="p-4">Move-in</th><th className="p-4">Phone</th></tr></thead><tbody>{typedLeads.map(lead=><tr key={lead.id} className="border-t border-ink/5"><td className="p-4 font-semibold">{lead.name}<div className="font-normal text-ink/45">{lead.preferred_area}</div></td><td className="p-4 capitalize">{lead.requested_property_type}</td><td className="p-4">${Number(lead.budget).toLocaleString()}</td><td className="p-4">{lead.move_in_date || "Flexible"}</td><td className="p-4">{lead.phone}</td></tr>)}</tbody></table></div>}
      </div>
    </section>
  </main>;
}
