...
export default async function ProjectsPage() {
  ...
  return <main className="mx-auto max-w-7xl px-5 py-12 sm:px-6 sm:py-16"><div className="max-w-3xl"><p className="eyebrow">Developments</p><h1 className="mt-3 text-4xl font-black tracking-tight sm:text-5xl">Explore projects, project by project.</h1><p className="mt-5 text-lg leading-8 text-ink/55">See every available unit, gallery, investment detail, and completion timeline in one place.</p></div>
    {grouped.size ? <div className="mt-10 grid gap-6 md:grid-cols-2">{Array.from(grouped.entries()).map(([name, listings]) => { const cover = listings.find(item => item.images?.length)?.images[0]; const fromPrice = Math.min(...listings.map(item => Number(item.price))); return <Link href={`/projects/${encodeURIComponent(name)}`} key={name} className="card group overflow-hidden"><div className="relative aspect-[16/8] overflow-hidden bg-gradient-to-br from-ink to-sage">{cover ? <Image src={cover} alt={`${name} project`} fill unoptimized className="object-cover transition duration-500 group-hover:scale-105"/> : <div className="flex h-full items-center justify-center px-5 text-center text-2xl font-black text-white">{name}</div>}</div><div className="p-6"><p className="text-xs font-bold uppercase text-sage">{listings.length} available unit{listings.length === 1 ? "" : "s"}</p><h2 className="mt-2 text-2xl font-black">{name}</h2><p className="mt-2 text-sm text-ink/50">From ${fromPrice.toLocaleString()}</p></div></Link>; })}</div> : <div className="card mt-10 p-8 text-center sm:p-12"><h2 className="text-xl font-bold">Projects are coming soon.</h2><p className="mt-2 text-sm leading-6 text-ink/50">Add a project name to listings in the AI Listing Studio.</p></div>}
  </main>;
}
