...
export default async function InvestmentsPage() {
  const { data } = await (await createClient()).from("properties").select("*").eq("availability", "available").eq("investment_opportunity", true).order("expected_roi", { ascending: false, nullsFirst: false });
