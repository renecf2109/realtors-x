...
export function ListingsManager({ initialProperties, currentUserId }: { initialProperties: Property[]; currentUserId: string }) {
  ...
  return <section>
    ...
    <div className="mt-8 grid gap-4 md:grid-cols-2">{items.map(property => <article key={property.id} className="card overflow-hidden">
      ...
      <div className="mt-5 flex gap-2 border-t border-ink/10 pt-4"><button onClick={() => { setEditing(property); setOpen(true); }} className="btn-secondary gap-2 !px-4 !py-2"><Pencil size={14}/> Edit</button>{property.agent_id === currentUserId && <button onClick={() => remove(property.id)} className="btn-secondary gap-2 !px-4 !py-2 text-red-600"><Trash2 size={14}/> Delete</button>}</div></div>
    </article>)}</div>
    ...
    <div className="mx-auto my-3 max-w-4xl rounded-3xl bg-cream p-5 shadow-soft sm:p-8">
      ...
      {step === "start" ? <div className="mt-7 grid gap-5 md:grid-cols-2">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Developer name (agents only by default)"><input className="field" value={draft.developer_name ?? ""} onChange={e => update("developer_name", e.target.value || null)} placeholder="Developer or company"/></Field>
          <label className="flex items-center gap-3 rounded-2xl border border-ink/10 bg-white p-4 text-sm font-semibold"><input type="checkbox" className="h-5 w-5 accent-sage" checked={draft.show_developer_to_public ?? false} onChange={e => update("show_developer_to_public", e.target.checked)}/><span>Allow clients to see developer name</span></label>
